...
const CACHE_NAME = 'examai-pro-offline-v2';
const STATIC_ASSETS = [
  '/',
  '/index.html',
  '/manifest.json',
...
self.addEventListener('install', (event) => {
  ...
  event.waitUntil(
    caches.open(CACHE_NAME).then((cache) => {
      // cache.addAll is atomic, if one fails, all fail. 
      // These external assets are critical for design.
  ...
  event.respondWith(
    caches.match(event.request).then((cachedRes) => {
      ...
      return fetch(event.request).then((networkRes) => {
        ...
        // Check for both basic HTTP/S requests and external resources
